...
Type TUniFont
	...
	End Rem
	Method DrawTextBank(TMP_TextBank:TBank, TMP_X:Float = 0, TMP_Y:Float = 0)
		...
		For Local TMP_Count:Int = 0 To (BankSize(TMP_TextBank) - 2) / 2 - 1
			...
			Else
				...
				If (TMP_Font:TImage)
					DrawImage(TMP_Font:TImage, TMP_X:Int + (ImageWidth(TMP_Font:TImage) * (TMP_Count - CharSkip)), TMP_Y:Int + (ImageHeight(TMP_Font:TImage) * LineCount), TMP_Letter:Byte)
				Else
	...
	End Rem
	Method DrawText(Text:String, TMP_X:Float, TMP_Y:Float)
		Local LineCount:Int = 0	'Multilines
		Local CharSkip:Int = 0 'Basically scrolls the X position backwards, it's used on multilines
